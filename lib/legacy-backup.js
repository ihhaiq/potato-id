import { db } from 'sdk';
import { botSettings, commandAliases, heartVotes, usageUsers } from 'schema';
import { seededEffectiveConfig } from 'lib/config';
...
function normalizeHearts(raw) {
  ...
  for (const [targetRaw, voters] of Object.entries(likes)) {
    ...
    for (const voterRaw of voters) {
      ...
      seen.add(key);
      rows.push({ key, targetUserId: target, voterUserId: voter });
    }
  }
  return rows;
}
...
export async function importLegacyBackup(payload) {
  ...
  for (const row of normalized.hearts) {
  ...
  return {
    ...
    hearts: normalized.hearts.length,
